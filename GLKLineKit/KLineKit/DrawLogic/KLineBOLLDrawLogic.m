...
- (void)drawWithCGContext:(CGContextRef)ctx rect:(CGRect)rect indexPathForVisibleRange:(CGPoint)visibleRange scale:(CGFloat)scale otherArguments:(NSDictionary *)arguments {
    ...
    // up
    [self p_drawLineWithPointArray:self.upPointArray atContent:ctx color:[self.config ma5Color].CGColor];
    
    // mid
    [self p_drawLineWithPointArray:self.midPointArray atContent:ctx color:[self.config ma10Color].CGColor];
