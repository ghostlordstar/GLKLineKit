...
/**
 数据准备状态集合的递归锁
 */
@property (strong, nonatomic) NSRecursiveLock *stateDictLock;
...
- (NSRecursiveLock *)stateDictLock {
    if (!_stateDictLock) {
        _stateDictLock = [[NSRecursiveLock alloc] init];
        _stateDictLock.name = @"indicatorsDataStateDict RecursiveLock";
    }
    return _stateDictLock;
}
