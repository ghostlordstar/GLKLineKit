...
- (void)drawRect:(CGRect)rect {
    ...
    [NSObject gl_drawTextBackGroundInRect:rect content:ctx boderColor:[UIColor whiteColor] boderWidth:1.0f fillColor:[[UIColor grayColor] colorWithAlphaComponent:0.8]];
    
    for (NSInteger a = 0; a < self.contentArray.count; a ++) {
        DetailDataModel *tempModel = self.contentArray[a];
        CGRect textRect = CGRectMake(rect.origin.x, a * self.rowHeight , rect.size.width, self.rowHeight);
